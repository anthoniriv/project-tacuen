...
export function AllocationEditor({
  ...
  const handleModeChange = (newMode: AllocationMode) => {
    ...
    switch (newMode) {
      ...
      case "fixed_amount":
        ...
        if (people.length > 0) {
          defaultAmounts[people[0].id] = item.total;
        }
        newAllocation = {
          itemId: item.id,
          mode: "fixed_amount",
  ...
  const handleFixedAmountChange = (personId: string, amount: number) => {
    if (mode !== "fixed_amount") return;

    const fixedAmounts = {
      ...(allocation.fixedAmounts || {}),
      [personId]: Math.max(0, amount),
    };
  ...
  return (
    <div className="w-full space-y-4">
      ...
      {mode === "fixed_amount" && allocation.fixedAmounts && (
        <div className="space-y-2">
          <label className="block text-xs font-medium text-neutral-400 mb-2">
            Montos fijos (Total disponible: {item.total.toFixed(2)})
          </label>
          {allocation.participants.map((personId) => {
            const person = people.find((p) => p.id === personId);
            if (!person) return null;
            return (
              <div key={personId} className="flex items-center gap-2">
                ...
                <input
                  ...
                  min="0"
                  max={item.total}
                  value={allocation.fixedAmounts?.[personId] || 0}
                  onChange={(e) =>
